...
Local Instance N_proper : Proper (Aeq ==> Aeq) N.
Proof.
...
Global Instance in_elem_of: ElemOf set set := IN.
Lemma IN_unfold (X Y: set): X ∈ Y = Ain X Y.
Proof. reflexivity. Qed.


Definition Subq (X Y : set) := ∀ Z, Z ∈ X → Z ∈ Y.
Local Instance Subq_subseteq: SubsetEq set := Subq.
...
Global Instance empty_set_notation: Empty set := empty_set.
...
Global Instance singleton_Singleton: Singleton set set := singleton_set.

Definition bunion_set A B := union_set (upair_set A B).
Global Instance bunion_Union : Union set := bunion_set.
...
Global Instance binter_notation : Intersection set := binter_set.
