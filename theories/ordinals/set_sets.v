...
Require Export category.ordinals.set_model.
...
Local Arguments union_set : simpl never.
Local Arguments replacement_set : simpl never.
Local Arguments desc_set: simpl never.
Local Arguments upair_set : simpl never.
Local Arguments specification_set : simpl never.
Local Arguments power_set : simpl never.
...
- apply zf_union. apply empty_el in J as [A H]. exists A. split.
  + exact (I A H).
  + assumption.
- assumption.
Qed.
...
- apply zf_union. exists A. split.
  + apply I.
  + apply single_el. reflexivity.
Qed.
...
- subst b. apply single_el in J. subst a. cut (A=B).
  + intros H. subst A. assumption.
  + apply opair_intuni. apply J'. apply interI.
    * intros a H. apply zf_pair in H as [H|H]; subst a.
      -- by apply single_el.
      -- apply zf_pair. eauto.
  * apply empty_el. exists (singleton A). apply upair1.
...
split; intros I.
- split.
  + rewrite <- (pi1_cor A B), <- (pi1_cor A' B'). by rewrite I.
...
* apply single_el in I. apply zf_replacement. exists a. split.
  -- assumption.
...
- destruct I as [a [b [A' [B' P]]]]. apply zf_union.
  ...
  + apply zf_replacement. exists b. split.
    * apply B'.
    * assumption.
  + apply zf_replacement. exists a. split.
    * apply A'.
    * reflexivity.
Qed.
