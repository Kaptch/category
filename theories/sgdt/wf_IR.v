...
Require Import ordinals.stepindex.
...
Section IR.
  ...
  Variable (rel_pi : ∀ α β, ProofIrrel (rel α β)).
  Notation "a ≺ b" := (rel a b).
  Notation "a ⪯ b" := (rc rel a b).
  Variable (rel_pi' : ∀ α β, ProofIrrel (α ⪯ β)).
  ...
  Variable (le_lt_eq_dec : ∀ α β, α ⪯ β → {α ≺ β} + {α = β}).
  ...
  Variable (A_agree : ∀ {P1 P2}, A P1 → A P2 → Type).
  Variable (A_agree_transitive : ∀ (P0 P1 P2 : X → Prop)
                                   (H0 : ∀ x, ProofIrrel (P0 x))
                                   (H1 : ∀ x, ProofIrrel (P1 x))
                                   (H2 : ∀ x, ProofIrrel (P2 x))
                                   A0 A1 A2, (∀ γ, P0 γ → P2 γ → P1 γ)
      → @A_agree P0 P1 A0 A1 → @A_agree P1 P2 A1 A2 → @A_agree P0 P2 A0 A2).
  Variable (A_agree_symmetric : ∀ (P0 P1 : X → Prop)
                                  (H0 : ∀ x, ProofIrrel (P0 x))
                                  (H1 : ∀ x, ProofIrrel (P1 x))
                                  A0 A1, @A_agree P0 P1 A0 A1 → @A_agree P1 P0 A1 A0).
  ...
  Inductive IR_spec : ∀ (x : X), A (λ y, y ⪯ x) → Type :=
  ...
  Proof using step_preserve step_agree rel_wf rel_transitive le_lt_eq_dec merge_preserve merge_agree existT_X_inj2 A_agree_transitive rel_pi rel_pi'.
    revert x2 x1 A1 A2. refine (Fix rel_wf _ _). intros x2 IH x1 A1 A2 Hle H1 H2.
    destruct (le_lt_eq_dec _ _ Hle).
    - subst. inversion H2. subst. apply existT_X_inj2 in H3. subst.
      eapply A_agree_transitive.
      6: { eapply step_agree. }
      1: intros; apply rel_pi'.
      1: intros; apply rel_pi.
      1: intros; apply rel_pi'.
      2: { eapply A_agree_transitive.
           6: { apply merge_agree. }
           intros; apply rel_pi'.
           intros; apply rel_pi'.
           intros; apply rel_pi.
  ...
  Proof using A A_agree A_agree_symmetric A_agree_transitive X existT_X_inj2 le_lt_eq_dec
merge_agree merge_preserve rel rel_rc_total rel_transitive rel_wf step step_agree step_merge
step_preserve rel_pi rel_pi'.
    intros H1 x0 x1 Hx0 Hx1.
    destruct (rel_rc_total x0 x1) as [H2 | H2].
    - apply IR_spec_unique; eauto.
    - apply A_agree_symmetric. 1-2: intros; apply rel_pi'. apply IR_spec_unique; eauto.
  Qed.
...
step_preserve rel_pi rel_pi'.
  ...
  Proof.
    ...
    - apply A_agree_symmetric. 1-2: intros; apply rel_pi'. apply IR_spec_unique; eauto using A_all_p.
  Qed.
...
Section IR_wf_index.
  ...
  Variable (A_agree : ∀ {P1 P2}, A P1 → A P2 → Type).
  Variable (A_agree_transitive : ∀ (P0 P1 P2 : SI → Prop)
                                   (H0 : ∀ x, ProofIrrel (P0 x))
                                   (H1 : ∀ x, ProofIrrel (P1 x))
                                   (H2 : ∀ x, ProofIrrel (P2 x))
                                   A0 A1 A2, (∀ γ, P0 γ → P2 γ → P1 γ)
      → @A_agree P0 P1 A0 A1 → @A_agree P1 P2 A1 A2 → @A_agree P0 P2 A0 A2).
  Variable (A_agree_symmetric : ∀ (P0 P1 : SI → Prop)
                                  (H0 : ∀ x, ProofIrrel (P0 x))
                                  (H1 : ∀ x, ProofIrrel (P1 x))
                                  A0 A1, @A_agree P0 P1 A0 A1 → @A_agree P1 P0 A1 A0).
...
Section IR_transfinite_index_cons.
  ...
  Variable (A_agree_symmetric : ∀ (P0 P1 : SI → Prop)
                                  ...
                                  (H1 : ∀ x, ProofIrrel (P1 x))
                                  A0 A1, @A_agree P0 P1 A0 A1 → @A_agree P1 P0 A1 A0).
  Variable (A_agree_reflexive : ∀ (P : SI → Prop) (H0 : ∀ x, ProofIrrel (P x)) A, @A_agree P P A A).
  ...
  Variable (E_agree : ∀ γ ap0 ap1, E γ ap0 → E γ ap1 → A_agree ap0 ap1 → Type).
